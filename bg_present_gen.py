...
def gen_present_files() :
	...
	for i, quiver in enumerate( open( fileDict['quiver_file'], 'r' ) ) :
		...
		try :
			fspot = open( fileDict['bg_spot_file'] % (i+1), 'r' )
			outFile = fileDict['bg_m2out_file'] % (i+1)
			presentFile = fileDict['bg_present_file'] % (i+1)
		except :
			try :
				fspot = open( fileDict['bg_spot_r_file'] % (i+1), 'r' )
				outFile = fileDict['bg_m2out_r_file'] % (i+1)
				presentFile = fileDict['bg_present_r_file'] % (i+1)
			except :
	...
	write_to_dict( dimDict, fileDict['bg_dim_file'] )
	write_to_dict( degDict, fileDict['bg_deg_file'] )
...
if __name__ == '__main__' :
	...
	if not check_file( fileDict['count_file'], fileDict['quiver_file'] ) or not check_folder( False, fileDict['bg_spot_folder'], fileDict['bg_m2out_folder'], fileDict['gio_folder'] ) :
		print 'The quiver file or background superpotential files or M2Out files do not exist. Quit...'
		my_quit( 2 )
	check_folder( True, fileDict['bg_present_folder'] )

...
if __name__ == '__main__' :
	...
	if not check_file( fileDict['count_file'] ) or not check_folder( False, fileDict['present_folder'], fileDict['gio_folder'] ) :
		print "\n!!! Can not find count_file or present_folder or gio_folder. Quit..."
		my_quit( 1 )
	...
	for i in range( eval( open( fileDict['count_file'], 'r' ).read() ) ) :
		...
		try :
			fpr = open( fileDict['present_file'] % (i+1), 'r' )
		except :
			try :
				fpr = open( fileDict['present_r_file'] % (i+1), 'r' )
			except :
	...
	open( fileDict['deg-dim_file'], 'w' ).write( content )
	open( fileDict['deg-dim_math_file'], 'w' ).write( content.replace( '[', '{' ).replace( ']', '}' ) )

...
def write_to_wterm_file( wList, k ) :
	fh = open( fileDict['term_file'] % k, 'w' )
	for cycle in wList :
...
def all_potentials( k ) :
	...
	index, wList = 0, ''
	fh = open( fileDict['m2in_file'] % k, 'w' )
	...
	if wList :
		open( fileDict['spot_file'] % k, 'w' ).write( wList )
	return index
...
def random_potentials( k ) :
	...
	fh = open( fileDict['m2in_r_file'] % k, 'w' )
	...
	if wList :
		open( fileDict['spot_r_file'] % k, 'w' ).write( wList )
	return index
...
def gen_data_files() :
	...
	for count, cycles in enumerate( open( fileDict['quiver_file'], 'r' ) ) :
		...
		wTermList = select_Ws( globCycles )
		globGIOList = select_GIOs( globCycles )
		globBigGIOList = [ c for c in globGIOList if len( c ) > 2 ]
		
		write_to_wterm_file( wTermList, count+1 )
		write_to_gio_file( count+1 )
...
if __name__ == '__main__' :
	...
	if not check_file( fileDict['quiver_file'] ) :
		...
		my_quit( 2 )
	check_folder( True, fileDict['term_folder'], fileDict['gio_folder'], fileDict['spot_folder'], fileDict['m2in_folder'], fileDict['m2out_folder'] )
